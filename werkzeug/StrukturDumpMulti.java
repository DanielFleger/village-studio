// Wie StrukturDump, aber SHC_STRUKT darf mehrere Muster kommagetrennt enthalten.
import ghidra.app.script.GhidraScript;
import ghidra.program.model.data.*;
import java.io.PrintWriter;
import java.util.Iterator;

public class StrukturDumpMulti extends GhidraScript {
    @Override
    public void run() throws Exception {
        ...
        while (it.hasNext()) {
            Structure s = it.next();
            String n0 = s.getName().toLowerCase();
            boolean treffer = false;
            for (String t : teile) if (n0.contains(t.trim())) treffer = true;
            if (!treffer) continue;
            gefunden++;
            ...
            for (DataTypeComponent c : s.getDefinedComponents()) {
                ...
                w.printf("  +0x%-5s %-46s %-24s %d Byte%n",
