...
// Wie tief die Krone in den Koerper einsinkt. GEMESSEN, nicht geraten: die
// breiteste Zeile jeder Krone (tile_land3#96, #112, #120) liegt genau 7 Punkte
// ueber ihrer Unterkante. Genau diese Zeile gehoert auf die Schnittkante des
// Koerpers - sonst schauen dessen Ecken unter der Krone hervor. Daniel sah das
// als "leichten Cut" und als "kleines Dreieck, das unten absteht".
const AUFSATZ_UEBERLAPP = 7;

// Am Fuss der Mauerbilder liegt ein gemalter Bauschutthaufen - Teil der
// Bauvorschau, nicht der Mauer. Die Form dort ist sauber (die Kachelraute), nur
// die Farbe stimmt nicht. Darum werden die untersten Zeilen mit der Textur von
// genau einer Kachelhoehe weiter oben uebermalt: das Mauermuster laeuft nahtlos
// durch, die Kachelform bleibt.
const SCHUTT_ZEILEN = 16;

function ohneBauschutt(bild, zeilen = SCHUTT_ZEILEN) {
  if (!bild || bild.hoehe <= zeilen * 2) return bild;
  const rgba = Buffer.from(bild.rgba);
  for (let y = bild.hoehe - zeilen; y < bild.hoehe; y++) {
    for (let x = 0; x < bild.breite; x++) {
      const hier = (y * bild.breite + x) * 4;
      if (!rgba[hier + 3]) continue;                 // durchsichtig bleibt durchsichtig
      const oben = ((y - zeilen) * bild.breite + x) * 4;
      rgba[hier] = bild.rgba[oben]; rgba[hier + 1] = bild.rgba[oben + 1];
      rgba[hier + 2] = bild.rgba[oben + 2];
    }
  }
  return { breite: bild.breite, hoehe: bild.hoehe, rgba, kacheln: bild.kacheln };
}

// Die Kappe auf den Mauerbildern ist ein GERUEST, kein Mauerabschluss - ein
// Podest mit Loechern, durch das man im Editor hindurchsah. Gemessen an
// tile_buildings1#0: in den Zeilen 6 bis 12 stehen Loecher mitten im Bild
// ("#######....########....#######"), ab Zeile 13 ist es deckend. Diese
// dreizehn Zeilen fallen weg, dann kommt die echte Krone darauf.
// Daniel am 06.09.2026: "alle vier Mauertypen, kleine/grosse, mit/ohne Crenel,
// haben immer noch transparente Tops."
const GERUESTKAPPE = 13;

// Jede Mauer ist Koerper plus Krone. Ohne Zinnen ist die Krone eine flache
// Gehplatte, mit Zinnen wechseln Klotz und Scharte Feld fuer Feld - im Spiel
// entschieden an x+y ungerade (placeDefensiveStructureTile 0x005034a0).
const AUFSAETZE = {
  // Mapper 25 = AIV 10, Steinmauer
  25: { koerper: 'tile_buildings1#0', kappe: GERUESTKAPPE, klotz: 'tile_land3#96' },
  // Mapper 46 = AIV 11, Niedrige Mauer
  46: { koerper: 'tile_buildings1#1', kappe: GERUESTKAPPE, klotz: 'tile_land3#96' },
  // Mapper 26 = AIV 12, Zinnenmauer hoch
  26: { koerper: 'tile_buildings1#0', kappe: GERUESTKAPPE,
        klotz: 'tile_land3#120', scharte: 'tile_land3#112' },
  // Mapper 35 = AIV 13, Zinnenmauer niedrig
  35: { koerper: 'tile_buildings1#1', kappe: GERUESTKAPPE,
        klotz: 'tile_land3#120', scharte: 'tile_land3#112' },
};

// Die obersten Zeilen eines Bildes wegschneiden.
function ohneKappe(bild, zeilen) {
  if (!bild || !zeilen) return bild;
  const hoehe = bild.hoehe - zeilen;
  if (hoehe <= 0) return bild;
  const rgba = Buffer.alloc(bild.breite * hoehe * 4);
  bild.rgba.copy(rgba, 0, zeilen * bild.breite * 4);
  return { breite: bild.breite, hoehe, rgba, kacheln: 1 };
}
...
module.exports = { bilderIndex, bildPng, pngVon, pruefstand, vorplaetze, aufsaetze, mitAufsatz, ohneKappe, ohneBauschutt, leseSkin, skinFuer, urteile,
