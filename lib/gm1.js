...
// ---- Ein ganzes Gebaeude ----
// Ein Eintrag ist genau EINE Rautenkachel breit (30 Punkte). Ein Gebaeude
// besteht aus `teile` Eintraegen, und `teile` ist eine Quadratzahl: n*n bei
// n x n Kacheln Grundflaeche.
//
// Wichtig, und hier lag mein erster Irrtum: Die Lage der Teile steht NICHT in
// der Datei. versatzX/versatzY zeigen woandershin. Die Lage wird gerechnet -
// die Teile fuellen eine Raute Zeile fuer Zeile, von der untersten Spitze nach
// oben: erst 1 Kachel, dann 2, 3 ... bis n, danach wieder abwaerts.
// Je Kachel 32 Punkte nach rechts, je Zeile 8 nach oben und 16 nach links
// (nach der Mitte 16 nach rechts).
//
// Der zweite Irrtum: `kachelVersatz` hebt den Aufbau ueber seine Kachel. Ohne
// dieses Feld schwebt das Dach. Bei Richtung 3 kommen 14 Punkte nach rechts.
//
// Nachgebaut nach Gm1KonverterCrossPlatform (PodeCaradox), dort DecodedFile.cs
// und TilesImage.cs - eigenes Herleiten hatte zweimal danebengelegen.

const PUFFER = 500;

function ganzesGebaeude(g, startNr) {
  const s = g.bilder[startNr];
  const n = Math.round(Math.sqrt(s.teile));
  const teile = [];
  ...
  const B = n * 30 + (n - 1) * 2;
  const H = n * 16 + (letzter.kachelVersatz || 0) + PUFFER;
  ...
  let offsetY = H - 16;
  let offsetX = Math.floor(n / 2) * 30 + (n - 1) - (n % 2 === 0 ? 15 : 0);
  let sicher = offsetX, jeZeile = 1, inZeile = 0, haelfteErreicht = false;
  let obersteZeile = H;

  for (const b of teile) {
    const roh = g.buf.subarray(g.aBilder + b.offset, g.aBilder + b.offset + b.groesse);

    // Aufbau zuerst, er liegt hinter der eigenen Kachel
    if (roh.length > 512) {
      const rechts = b.richtung === 3 ? 14 : 0;
      const oy = offsetY - (b.kachelVersatz || 0);
      if (oy < obersteZeile) obersteZeile = oy;
      const auf = tgxNachRgba(roh.subarray(512), 30, PUFFER, null);
      for (let y = 0; y < PUFFER; y++) for (let x = 0; x < 30; x++) {
        const q = (y * 30 + x) * 4;
        if (!auf[q + 3]) continue;
        setze(offsetX + rechts + x, oy + y, [auf[q], auf[q + 1], auf[q + 2]]);
      }
    }
    ...
    for (let y = 0; y < KACHEL_H; y++) for (let x = 0; x < KACHEL_B; x++) {
      const q = (y * KACHEL_B + x) * 4;
      if (!k[q + 3]) continue;
      setze(offsetX + x, offsetY + y, [k[q], k[q + 1], k[q + 2]]);
    }
    if (offsetY < obersteZeile) obersteZeile = offsetY;
    ...
    if (inZeile === jeZeile) {
      ...
      offsetY -= 8;
      if (jeZeile < n) {
        if (haelfteErreicht) { jeZeile--; offsetX += 16; }
        else { jeZeile++; offsetX -= 16; }
      } else {
        jeZeile--; offsetX += 16; haelfteErreicht = true;
      }
      sicher = offsetX;
      inZeile = 0;
    }
  ...
  // oben abschneiden, was leer geblieben ist
  const von = Math.max(0, Math.min(obersteZeile, H));
  const hoehe = H - von;
  const zug = Buffer.alloc(B * hoehe * 4);
  bild.copy(zug, 0, von * B * 4, H * B * 4);
  return { breite: B, hoehe, rgba: zug, kacheln: n };
}
